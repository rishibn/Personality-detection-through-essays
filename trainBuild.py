...
class trainBuild:
	...
	def getValues(self):
		...
		self.data = pd.read_csv("D:/MCA matters/4th sem/minor/essays2.csv")
	...
	def getStatusProcessed(self):
		...
		self.data.to_csv("D:/MCA matters/4th sem/minor/clean/train_essayv1.csv", index = False, header = False)
		mat = [] ## store processed numerical vectors
		for index, row in self.data.iterrows():
			mat.append(status[index] + row[2:].values.tolist())
		## write to file
		pd.DataFrame(mat).to_csv("D:/MCA matters/4th sem/minor/clean/train_essay_v2.csv", index = False, header = False)
		print("its done")
x = trainBuild()
x.getValues()
